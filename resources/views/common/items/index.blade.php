...
<span class="new-button"><a href="{{ route('items.create') }}" class="btn btn-success btn-sm"><span class="fa fa-plus"></span>
        &nbsp;{{ trans('general.add_new') }}</a></span>
<span><a href="{{ route('import.create', ['common', 'items']) }}" class="btn btn-default btn-sm"><span class="fa fa-download"></span>
        &nbsp;{{ trans('import.import') }}</a></span>
@endpermission
<span><a href="{{ route('items.export', request()->input()) }}" class="btn btn-default btn-sm"><span class="fa fa-upload"></span>
            ...
            <table class="table table-striped table-hover" id="tbl-items">
                <thead>
                    <tr>
                        ...
                        <th class="col-md-2 text-right amount-space">@sortablelink('sale_price',
                            trans('items.sales_price'))</th>
                        <th class="col-md-2 hidden-xs text-right amount-space">@sortablelink('purchase_price',
                            trans('items.purchase_price'))</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="hidden-xs"><img src="{{ $item->picture ? Storage::url($item->picture->id) : asset('img/akaunting-logo-green.png') }}"
                        ...
                        <td class="text-right amount-space">{{ money($item->sale_price,
                            setting('general.default_currency'), true) }}</td>
                        <td class="hidden-xs text-right amount-space">{{ money($item->purchase_price,
                            setting('general.default_currency'), true) }}</td>
                        <td class="hidden-xs">
                            @if ($item->enabled)
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    ...
                                    @if ($item->enabled)
                                    <li><a href="{{ route('items.disable', $item->id) }}">{{ trans('general.disable')
                                            }}</a></li>
                                    @else
                                    ...
                                    @permission('create-common-items')
                                    <li class="divider"></li>
                                    <li><a href="{{ route('items.duplicate', $item->id) }}">{{
