    ...
    <div class="col-md-4">
        <div class="info-box">
            ...
            <div class="info-box-content">
                ...
                <span class="info-box-number">@money($total_incomes['total'], setting('general.default_currency'),
                    true)</span>
                <div class="progress-group" title="{{ trans('dashboard.open_invoices') }}: {{ $total_incomes['open_invoice'] }}<br>{{ trans('dashboard.overdue_invoices') }}: {{ $total_incomes['overdue_invoice'] }}"
    ...
    <div class="col-md-4">
        <div class="info-box">
            ...
            <div class="info-box-content">
                ...
                <span class="info-box-number">@money($total_expenses['total'], setting('general.default_currency'),
                    true)</span>

                <div class="progress-group" title="{{ trans('dashboard.open_bills') }}: {{ $total_expenses['open_bill'] }}<br>{{ trans('dashboard.overdue_bills') }}: {{ $total_expenses['overdue_bill'] }}"
                    data-toggle="tooltip" data-html="true">
                    <div class="progress sm">
    ...
    <div class="col-md-4">
        <div class="info-box">
            ...
            <div class="info-box-content">
                ...
                <div class="progress-group" title="{{ trans('dashboard.open_profit') }}: {{ $total_profit['open'] }}<br>{{ trans('dashboard.overdue_profit') }}: {{ $total_profit['overdue'] }}"
                    data-toggle="tooltip" data-html="true">
                    <div class="progress sm">
        ...
        <div class="box box-success">
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    <button type="button" id="cashflow-monthly" class="btn btn-default btn-sm">{{
                        trans('general.monthly') }}</button>&nbsp;&nbsp;
                    <button type="button" id="cashflow-quarterly" class="btn btn-default btn-sm">{{
                        trans('general.quarterly') }}</button>&nbsp;&nbsp;
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-left">{{ $item->category ? $item->category->name :
                                trans_choice('general.invoices', 2) }}</td>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-left">{{ Date::parse($item->paid_at)->format($date_format) }}</td>
                            <td class="text-left">{{ $item->category ? $item->category->name :
                                trans_choice('general.bills', 2) }}</td>
                            <td class="text-right">@money($item->amount, $item->currency_code, true)</td>
                        </tr>
                        @endforeach
                    </tbody>
...
<script type="text/javascript" src="{{ asset('js/moment/locale/' . strtolower(app()->getLocale()) . '.js') }}"></script>
@elseif (is_file(base_path('public/js/moment/locale/' . language()->getShortCode() . '.js')))
<script type="text/javascript" src="{{ asset('js/moment/locale/' . language()->getShortCode() . '.js') }}"></script>
@endif
    ...
    $(function () {
        ...
        $('#cashflow-range').daterangepicker({
            ...
            ranges: {
                ...
                '{{ trans("reports.previous_year") }}': [moment().subtract(1, 'year').startOf('year'),
                    moment().subtract(1, 'year').endOf('year')
                ],
                '{{ trans("reports.this_quarter") }}': [moment().subtract(2, 'months').startOf('month'),
                    moment().endOf('month')
                ],
                '{{ trans("reports.previous_quarter") }}': [moment().subtract(5, 'months').startOf(
                    'month'), moment().subtract(3, 'months').endOf('month')],
                '{{ trans("reports.last_12_months") }}': [moment().subtract(11, 'months').startOf(
    ...
    $(document).ready(function () {
        $('#cashflow-range').on('apply.daterangepicker', function (ev, picker) {
            ...
            $.ajax({
                ...
                type: 'get',
                dataType: 'html',
                data: 'period=' + period + '&start=' + picker.startDate.format('YYYY-MM-DD') +
        ...
        $('#cashflow-monthly').on('click', function () {
            ...
            $.ajax({
                ...
                type: 'get',
                dataType: 'html',
                data: 'period=month&start=' + picker.startDate.format('YYYY-MM-DD') + '&end=' +
        ...
        $('#cashflow-quarterly').on('click', function () {
            ...
            $.ajax({
                ...
                type: 'get',
                dataType: 'html',
                data: 'period=quarter&start=' + picker.startDate.format('YYYY-MM-DD') + '&end=' +
                    picker.endDate.format('YYYY-MM-DD'),
                success: function (data) {
                    $('#cashflow').html(data);
                }
