<tr id="item-row-{{ $item_row }}">
    ...
    <td class="text-center" style="vertical-align: middle;">
        ...
        <button type="button" onclick="$(this).tooltip('destroy'); $('#item-row-{{ $item_row }}').remove(); totalItem();"
            data-toggle="tooltip" title="{{ trans('general.delete') }}" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></button>
    ...
    <td {!! $errors->has('item.' . $item_row . '.name') ? 'class="has-error"' : '' !!}">
        ...
        <input value="{{ empty($item) ? '' : $item->name }}" class="form-control typeahead" required="required"
            placeholder="{{ trans('general.form.enter', ['field' => trans_choice('invoices.item_name', 1)]) }}" name="item[{{ $item_row }}][name]"
    ...
    <td {{ $errors->has('item.' . $item_row . '.quantity') ? 'class="has-error"' : '' }}">
        ...
        <input value="
        {{ empty($item) ? '' : $item->quantity }}" class=" form-control text-center" required="required" name="item[{{ $item_row }}][quantity]"
    ...
    <td {{ $errors->has('item.' . $item_row . 'price') ? 'class="has-error"' : '' }}">
        ...
        <input value="
        {{ empty($item) ? '' : $item->price }}" class=" form-control text-right input-price" required="required" name="item[{{ $item_row }}][price]"
    ...
    <td {{ $errors->has('item.' . $item_row . '.tax_id') ? 'class="has-error"' : '' }}">
        ...
        {!! $errors->first('item.' . $item_row . '.tax_id', '<p class="
        help-block">:message</p>') !!}
