...
<div class="box box-success">
    ...
    <div class="box-body">
        ...
        <div id="customer-create-user" class="form-group col-md-12 margin-top">
            @if ($customer->user_id)
            <strong>{{ trans('customers.user_created') }}</strong> &nbsp; {{ Form::checkbox('create_user', '1', 1,
            ['id' => 'create_user', 'disabled' => 'disabled']) }}
            @else
            <strong>{{ trans('customers.allow_login') }}</strong> &nbsp; {{ Form::checkbox('create_user', '1', null,
            ['id' => 'create_user']) }}
            @endif
        </div>
        ...
        $('#create_user').on('ifClicked', function (event) {
            ...
            if ($(this).prop('checked')) {
                ...
                if (!email) {
                    ...
                    $('input[name="email"]').parent().after('<p class="help-block">{{ trans('
                        validation.required ', ['
                        attribute ' => '
                        email ']) }}</p>');
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        $('.iCheck-helper').parent().after(
                            '<i class="fa fa-spinner fa-pulse fa-fw loading" style="margin-left: 10px;"></i>'
                        );
                    ...
                    success: function (json) {
                        if (json['errors']) {
                            if (json['data']) {
                                $('input[name="email"]').parent().parent().addClass(
                                    'has-error');
                                $('input[name="email"]').parent().after(
                                    '<p class="help-block">' + json['data'] + '</p>');
                                $('input[name="email"]').focus();
                            ...
                            $.ajax({
                                ...
                                complete: function () {
                                    $('.box-footer .btn').attr('disabled',
                                        false);
                                    $('.loading').remove();
                                },
                                success: function (json) {
                                    $('#customer-create-user').after(json[
                                        'html']);
                                }
                            });
                        }
