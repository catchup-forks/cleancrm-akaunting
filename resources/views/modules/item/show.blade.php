...
@section('new_button')
<span class="new-button"><a href="{{ url('apps/token/create') }}" class="btn btn-success btn-sm"><span class="fa fa-key"></span>
        &nbsp;{{ trans('modules.api_token') }}</a></span>
<span class="new-button"><a href="{{ url('apps/my')  }}" class="btn btn-default btn-sm"><span class="fa fa-user"></span>
        &nbsp;{{ trans('modules.my_apps') }}</a></span>
@endsection
        ...
        <div class="col-md-8 no-padding-left">
            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="#description" data-toggle="tab" aria-expanded="true">{{
                            trans('general.description') }}</a></li>
                    @if ($module->installation)
                    <li class=""><a href="#installation" data-toggle="tab" aria-expanded="false">{{
                            trans('modules.tab.installation') }}</a></li>
                    @endif
                    @if ($module->faq)
                    <li class=""><a href="#faq" data-toggle="tab" aria-expanded="false">{{ trans('modules.tab.faq') }}</a></li>
                    @endif
                    @if ($module->changelog)
                    <li class=""><a href="#changelog" data-toggle="tab" aria-expanded="false">{{
                            trans('modules.tab.changelog') }}</a></li>
                    @endif
                </ul>
        ...
        <div class="col-md-4">
            ...
            <div class="box box-success">
                ...
                <div class="box-footer">
                    ...
                    <a href="{{ url('apps/' . $module->slug . '/uninstall') }}" class="btn btn-block btn-danger">{{
                        trans('modules.button.uninstall') }}</a>
                    @endpermission
                    @permission('update-modules-item')
                    @if ($enable)
                    <a href="{{ url('apps/' . $module->slug . '/disable') }}" class="btn btn-block btn-warning">{{
                        trans('modules.button.disable') }}</a>
                    @else
                    <a href="{{ url('apps/' . $module->slug . '/enable') }}" class="btn btn-block btn-success">{{
                        trans('modules.button.enable') }}</a>
                    @endif
                    @endpermission
                    @else
                    ...
                    <table class="table table-striped">
                        <tbody>
                            <tr>
                                ...
                                <td class="text-right"><a href="{{ url('apps/vendor/' . $module->vendor->id) }}">{{
                                        $module->vendor_name }}</a></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="text-right"><a href="{{ url('apps/categories/' . $module->category->slug) }}">{{
                                        $module->category->name }}</a></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
<script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json['errorr']) {
                        ...
                        $('#progress-text').html('<div class="text-danger">' + json['error'] +
                            '</div>');
                    }
            ...
            setTimeout(function () {
                $.ajax({
                    ...
                    success: function (json) {
                        if (json['errors']) {
                            $('#progress-bar').addClass('progress-bar-danger');
                            $('#progress-text').html('<div class="text-danger">' + json['errors'] +
                                '</div>');
                        }
    ...
    function startInstallation() {
        ...
        modal += '              <h4 class="modal-title">{{ trans('
        modules.installation.header ') }}</h4>';
        modal += '          </div>';
        ...
        modal += '                      <span class="sr-only">{{ trans('
        modules.installation.start ', ['
